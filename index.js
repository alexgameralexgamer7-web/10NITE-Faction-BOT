...
const ROLE_ACCEPT_ID = '1490441838429999134';
...
client.on('interactionCreate', async (interaction) => {
    ...
    if (interaction.commandName === 'accept') {
        ...
        try {
            ...
            await membreGuild.roles.add(ROLE_ACCEPT_ID);
            await salon.send({ content: `${membre}`, embeds: [embed] });
            await interaction.reply({ content: `✅ Candidature de ${membre} acceptée et rôle attribué !`, ephemeral: true });
        } catch (error) {
            if (error.code === 10007) {
                await interaction.reply({ 
                    content: `❌ Impossible d'ajouter le rôle : ${membre} n'est pas dans le serveur.`, 
                    ephemeral: true 
                });
            } else {
                await interaction.reply({ 
    ...
    if (interaction.commandName === 'refuse') {
        ...
        try {
            ...
            await interaction.reply({ 
                content: `❌ Erreur lors du refus : ${error.message}`, 
                ephemeral: true 
            });
        }
    }
